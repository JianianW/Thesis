...
def train_model(train_data, val_data, model, optimizer, device, mixup=False, alpha=1.0, patience=500, max_epochs=1000, batch_size=32):
    ...
    num_classes = model.linears_prediction[0].out_features
    ...
    for epoch in range(max_epochs):
        ...
        for data in train_loader:
            ...
            if mixup:
                ...
                mixed_h = model.linears_prediction[-1](mixed_h)
...
def test_model(test_data, model, device):
    ...
    test_loader = DataLoader(test_data, batch_size=32, num_workers=0, pin_memory=False)
    num_classes = model.linears_prediction[0].out_features
    with torch.no_grad():
...
# Function to perform 50 independent trials per fold
def train_with_early_stopping(train_data, test_data, model, optimizer, device, mixup=False, alpha=1.0, patience=500, max_epochs=1000, batch_size=32, fold_idx=0):
    ...
    loss_dir = os.path.join(os.getcwd(), "loss_gin")
    ...
    for trial in tqdm(range(50), desc="Independent Trials"):
        ...
        # Train the model
        best_model_state, train_loss_history, val_loss_history = train_model(train_split, val_split, model, optimizer, device, mixup, alpha, patience, max_epochs, batch_size)
